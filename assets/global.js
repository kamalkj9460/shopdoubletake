...
class VariantSelects extends HTMLElement {
  ...
  onVariantChange() {
    ...
    if (!this.currentVariant) {
      ...
      this.updateMedia();
 // TTT custom method filter thumbnails
      this.filterMedia()
      this.updateURL();
...
//     if(variantQuantity.textContent <= 0) {
//       addButtonText.textContent = variantQuantity.dataset.preorder;
//       if(!document.querySelector('.preorder-message')) return;
//       document.querySelector('.preorder-message').classList.add('show_note');   
//       dynamicBtn.classList.add('hidden');
//     } else {
...
//   }
  ...
  // TTT 4.19.22 custom code method filter thumbnails on product page to only show selected variant images
  filterMedia() {
     // hide all thumbnails
   const allThumbnails = document.querySelectorAll('[data-thumbnail-color]');
   allThumbnails.forEach((thumbnail) => {
     thumbnail.style.display = "none"
   });
   if (this.currentVariant.featured_media != null && this.currentVariant.featured_media.alt != null) {
    // reload hide all on page refresh to get rid of featured image
      allThumbnails.forEach((thumbnail) => {
        thumbnail.style.display = "none"
      });
      const selected_variant = this.currentVariant.featured_media.alt;

    document.querySelectorAll('[data-thumbnail-color="' + selected_variant + '"]').forEach((e) => {
        e.style.display="";
        })
    } else {
      allThumbnails.forEach((thumbnail) => {
   ...
   // end custom code method filter thumbnails on product page to only show selected variant images
  ...
  toggleAddButton(disable = true, text, modifyClass = true) {
    ...
    if(variantQuantity.dataset.availability == 'true' ) {
      if(variantQuantity.textContent <= 0) {
        addButtonText.textContent = variantQuantity.dataset.preorder;
        if(!document.querySelector('.preorder-message')) return;
        document.querySelector('.preorder-message').classList.add('show_note');   
      ...
      if(dynamicBtn) {
        dynamicBtn.classList.remove('hidden');
      }
    }
